...
/*
    ...
    func sendDataToDatabase() {
        ...
        newPostReference.setValue(["uid": currentUserId, "title": titleTextField.text!, "category": categoryTextField.text!, "tags": tagsTextField.text!, "numOfVacan": numOfVacanTextField.text!, "duration": durationTextField.text!, "location": locationTextField.text!, "description": descriptionTextView.text! ], withCompletionBlock: {
            (error, ref) in
            if error != nil {
                ProgressHUD.showError(error!.localizedDescription)
    ...
    @objc func textFieldDidChange() {
         guard let titleText = titleTextField.text, !titleText.isEmpty, let categoryText = categoryTextField.text, !categoryText.isEmpty, let tagsText = tagsTextField.text, !tagsText.isEmpty, let numOfVacanText = numOfVacanTextField.text, !numOfVacanText.isEmpty, let durationText = durationTextField.text, !durationText.isEmpty, let locationText = locationTextField.text, !locationText.isEmpty
            else {
