...
/*
    ...
    func sendDataToDatabase() {
        ...
        newPostReference.setValue(["uid": currentUserId, "title": titleTextField.text!, "category": categoryTextField.text!, "tags": tagsTextField.text!, "numOfVacan": numOfVacanTextField.text!, "location": locationTextField.text!, "time": timeTextField.text!, "description": descriptionTextView.text! ], withCompletionBlock: {
            (error, ref) in
            if error != nil {
                ProgressHUD.showError(error!.localizedDescription)
    ...
    @objc func textFieldDidChange() {
         guard let titleText = titleTextField.text, !titleText.isEmpty, let categoryText = categoryTextField.text, !categoryText.isEmpty, let tagsText = tagsTextField.text, !tagsText.isEmpty, let numOfVacanText = numOfVacanTextField.text, !numOfVacanText.isEmpty, let locationText = locationTextField.text, !locationText.isEmpty, let timeText = timeTextField.text, !timeText.isEmpty
            else {
